...
class SimulationResult:
    ...
    propagation_efficiency: float   # informed_count / transmissions
    avg_path_length: float          # topology-level descriptor
    avg_degree: float               # topology-level descriptor
    clustering_coeff: float         # topology-level descriptor
...
class BASimulator:
    ...
    def build_ba_graph(self, node_count: int, ba_m: int) -> nx.Graph:
        """
        Build a connected Barabasi-Albert scale-free graph.
        ba_m is the number of edges attached from a new node to existing nodes.
        """
        ba_m = max(1, min(ba_m, node_count - 1))
        g = nx.barabasi_albert_graph(node_count, ba_m, seed=self.seed)
        return g
    ...
    def build_cluster_view_from_ba(
        self,
        node_count: int,
        ch_count: int,
        ba_m: int,
        ...
        """
        Build BA graph, then derive cluster structure from the graph:
        - choose CHs as highest-degree nodes
        - assign each node to nearest CH (shortest-path distance)
        """
        ch_count = max(1, min(ch_count, node_count))
        g = self.build_ba_graph(node_count=node_count, ba_m=ba_m)
        ...
        sp_maps = {ch: nx.single_source_shortest_path_length(g, ch) for ch in cluster_heads}
        ...
        return g, cluster_heads, node_to_ch
    ...
    def run_gossip(
        ...
        for round_idx in range(1, max_rounds + 1):
            ...
            next_frontier: Set[Tuple[int, Optional[int]]] = set()

            # for u in frontier:
            for u, parent in frontier:
                if u not in active:
                    continue
    ...
    def run_cluster(
        ...
        # build CH overlay using shortest paths between CHs in the BA graph
...
# -----------------------------------------------------------------------------
# Experiment 1
# Fanout vs Propagation Performance / Duplication
# -----------------------------------------------------------------------------

def experiment_1_fanout_vs_duplication(out_dir: str = "results", runs: int = RUNS) -> List[dict]:
    rows: List[dict] = []
    node_count = 100
    ba_m = 3
    ...
    for fanout in [1, 2, 3, 4, 5, 6, 8]:
        ...
        rows.append(
            summarize_results(
                1,
                {"fanout": fanout, "node_count": node_count, "ba_m": ba_m},
                run_results,
            )
        )
...
# -----------------------------------------------------------------------------
# Experiment 2
# CH Count vs Node Count
# -----------------------------------------------------------------------------

def experiment_2_ch_count_vs_node_count(out_dir: str = "results", runs: int = RUNS) -> List[dict]:
    rows: List[dict] = []
    ...
    for node_count in [50, 100, 200, 300, 400]:
        ...
        for ch_count in ch_options:
            ...
            for i in range(runs):
                seed = BASE_SEED + i
                sim = BASimulator(seed)
                g, chs, node_to_ch = sim.build_cluster_view_from_ba(
...
# -----------------------------------------------------------------------------
# Experiment 3
# Topology Density Sensitivity
# In BA model, density is controlled by m
# -----------------------------------------------------------------------------

def experiment_3_topology_density_vs_performance(out_dir: str = "results", runs: int = RUNS) -> List[dict]:
    rows: List[dict] = []
    node_count = 100
    fanout = 3
    ...
    for ba_m in [1, 2, 3, 4, 6, 8]:
        ...
        rows.append(
            summarize_results(
                3,
                {"node_count": node_count, "ba_m": ba_m, "fanout": fanout},
                run_results,
            )
        )
...
# -----------------------------------------------------------------------------
# Experiment 4
# Cluster Head Overload / Failure
# -----------------------------------------------------------------------------

def experiment_4_ch_overload_failure(out_dir: str = "results", runs: int = RUNS) -> List[dict]:
    rows: List[dict] = []
    node_count = 120
    ch_count = 6
    ba_m = 3
...
def experiment_5_churn_sensitivity(out_dir: str = "results", runs: int = RUNS) -> List[dict]:
    rows: List[dict] = []
    node_count = 100
    ba_m = 3
    ...
    for churn_rate in [0.0, 0.05, 0.10, 0.20, 0.30, 0.40]:
        ...
        for i in range(runs):
            ...
            gossip_runs.append(sim.run_gossip(g=g, source=0, fanout=3, active_nodes=active))

            sim2 = BASimulator(seed)
            g2, chs, node_to_ch = sim2.build_cluster_view_from_ba(
                node_count=node_count,
                ch_count=5,
                ba_m=ba_m,
            )
            all_nodes2 = list(g2.nodes())
...
def experiment_6_heterogeneous_resources(out_dir: str = "results", runs: int = RUNS) -> List[dict]:
    rows: List[dict] = []
    node_count = 100
    ba_m = 3
    fanout = 4
...
def main() -> None:
    ...
    if RUN_MODE == "exp1":
        ...
        exp1 = experiment_1_fanout_vs_duplication(
            out_dir=out_dir,
            runs=RUNS,
        )
